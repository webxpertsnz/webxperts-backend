// api/reports/seo.js
//
// Real handler: accepts multipart/form-data with field "seo_file",
...
// try to parse a date-ish header cell
...
async function parseSeoWorkbook(filePath) {
  ...
  // Prefer sheet called "Ranking", otherwise first sheet
  ...
  // Try find domain/location in first 10 rows (optional)
  let domain = "";
  let location = "";
  for (let r = 1; r <= 10; r++) {
  ...
  // Find header row where first cell is "keyword"
  let headerRowNumber = null;
  rankingSheet.eachRow((row, rowNumber) => {
    const v = (row.getCell(1).value || "").toString().trim().toLowerCase();
    if (v === "keyword" && headerRowNumber === null) {
      headerRowNumber = rowNumber;
    }
  });

  if (!headerRowNumber) {
    throw new Error("Could not find a header row with 'Keyword' in column A.");
  }
  ...
  if (!dateCols.length) {
    throw new Error(
      "No date columns found in the header row. Make sure your ranking sheet has dates as column headings."
    );
  }
  ...
  if (!keywords.length) {
    throw new Error(
      "No keyword rows found under the header. Check that your ranking sheet has data below the 'Keyword' row."
    );
  }
